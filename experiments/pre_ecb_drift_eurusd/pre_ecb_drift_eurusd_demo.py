#!/usr/bin/env python3
"""pre_ecb_drift on EURUSD Phase 2 simulator + validation pipeline.

FX leg of the macro-event positioning-drift family.
Tests 24h pre-ECB-announcement LONG drift on EURUSD M5, 2019-01-02 to 2026-05-22.

Pre-committed kill criteria (from pre_ecb_drift_eurusd.md):
  - Per-trade mean > +0.05% (full sample, 1 bp RT)
  - W4 (2024-2026) per-trade mean > +0.03%
  - Win rate > 55%
  - MDD < 15%
  - Events >= 50
  - Direction null-gap (LONG - SHORT) >= +0.20
...
CAL_PATH = _ROOT / "experiments" / "pre_ecb_drift" / "ecb_calendar.csv"
EURUSD_M5_PATH = _ROOT / "ohlc_data" / "EURUSD_M5.csv"

# EURUSD typical Eightcap / IC Markets spread ~0.5-0.8 pip on 1.16 mid
# = 0.4-0.7 bps one-way = ~1 bp RT pessimistic.
...
# Sweep includes 0/1/2/5/10 — 5+ covers swap haircut on overnight FX hold.
...
# ---------- timezone (Europe/Berlin -> UTC, no pytz dependency) ----------
...
def cet_to_utc(local_dt: pd.Timestamp) -> pd.Timestamp:
    offset_h = 2 if cet_is_dst(local_dt) else 1
...
def regime_table(trades: pd.DataFrame) -> None:
    ...
    for w in ("W1", "W2", "W3", "W4"):
        ...
        marker = ""
        if w == "W4":
            if m["mean"] > 0.05: marker = "  <<< deploy-bar PASS"
            elif m["mean"] > 0.03: marker = "  <<< deploy-bar MARGINAL"
...
def placebo_check(df: pd.DataFrame, cal: pd.DataFrame,
    ...
    d = start
    while d <= end:
        if d.weekday() == 3 and d not in ecb_dates:
            all_thu.append(d)
    ...
    placebo_trades = compute_event_returns(df, fake_cal,
                                            cost_bps=cost_bps, direction="long")
...
def walk_forward(df: pd.DataFrame, cal: pd.DataFrame,
    ...
    for is_start, oos_start, oos_end in splits:
        ...
        is_trades  = compute_event_returns(df, is_cal,
                                            cost_bps=cost_bps, direction="long")
        oos_trades = compute_event_returns(df, oos_cal,
                                            cost_bps=cost_bps, direction="long")
...
def main() -> None:
    section("PRE-ECB DRIFT (EURUSD) -- Phase 2")
    ...
    # Baseline
    section("BASELINE -- 24h LONG, 1bp RT")
    ...
    if gap >= 0.20:
        print(f"  null-gap PASS (>=+0.20)")
    else:
        print(f"  null-gap FAIL (<+0.20) -- mechanism lacks directional content")
    ...
    if all_basic and null_ok and plc_ok and wf_ok:
        print("  PASS -- pre-ECB drift on EURUSD validated; proceed to event_calendar EA extension")
    elif (m_long['mean'] > 0.02) and null_ok and not all_basic:
